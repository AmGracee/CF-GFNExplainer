...
from src.utils.utils import linear_schedule,edges_motif_acc,contains_set



class GFlowNet:
    def __init__(self, gcn_model, sub_adj, sub_feat, sub_pred_orig, num_edges_orig,y_pred_orig, mask, env, replay, observations, device, delta=1.):
    ...
    def explain(self, graph_idx,prefill, num_iterations, optimizer, min_exploration, batch_size, indices, rng):
        ...
        for iteration in range(prefill+num_iterations):
            ...
            if cf_states != []:
                num_edges_min = min(cf_states[5]) # cf_states[5] is num_edges
                edges_min_idx = cf_states[5].index(num_edges_min)
                cf_example_min = [self.graph_idx,
                                  self.encode(cf_states[2][edges_min_idx]), # cf_states[2] is cf_adj
                                  cf_states[3],#sub_pred_orig
                                  cf_states[4][edges_min_idx], #cf_pred
                                  num_edges_min,
                                  cf_states[7][edges_min_idx],# order is the sequence of removing edges
                                  ]

                if cf_example_min[4] < best_num_edges:
                    best_cf_examples.append(cf_example_min)
                    best_num_edges = cf_example_min[4]
                    self.num_cf_examples += 1
                    print("iteration:{}, {} 个 CF examples for graph_idx = {}".format(iteration, self.num_cf_examples, self.graph_idx))
                    # break
                if len(terminal_state) != 0:
                    batch_terminal_state.append(terminal_state)
                    print('iteration:{},  Found terminal_state for graph_idx = {}'.format(iteration, self.graph_idx))

                if max(cf_states[6]).item() > best_output_change: # cf_states[6] is output_change
                    best_output_change = max(cf_states[6]).item()
                    fidelity_prob.append(best_output_change)

                for i in range(len(cf_states[2])):# cf_states[2] is cf_adj
                    diversity.append([cf_states[8][i], self.encode(cf_states[7][i].numpy())]) #cf_states[8][i]is perturb, cf_states[7][i]is order

        spent_time = (time.time()- start)/60 # time is minute.
        print("{} 个 CF examples for graph_idx = {}".format(self.num_cf_examples, self.graph_idx))
    ...
    def train(self, iteration):
        ...
        next_observations, rewards, dones, cf_states, terminal_state = self.env.step(self.graph_idx,self.sub_feat, self.sub_adj, self.sub_pred_orig, self.gcn_model, actions, self.num_edges_orig, logs)
        ...
        if iteration >= self.prefill:
            ...
            samples = self.replay.sample(self.batch_size, self.rng,self.device)
            ...
            print(
                ...
                'loss: {:.4f}'.format(loss),
                '{} 个 CF examples for graph_idx={}'.format(self.num_cf_examples, self.graph_idx))
            # print('===================================================')
